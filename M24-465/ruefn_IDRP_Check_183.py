"""
Rule Name: ruefn_IDRP_Check_183
Domains: LB_LOCAL_V
LBCAT = URINALYSIS and LBDTC <> Null and LBTESTCD <> Null  and LBORRES <> Null  and LBSTNRC = NULL. Query should fire.
"""

def ruefn_IDRP_Check_183(query_text, LB_LOCAL_V):
    payload_records = []
    ...
    if prim_df.shape[0] > 0:
        args_dict = {
			...
			'logic_text': "((LBCAT.str.upper() == 'URINALYSIS') and (LBDTC not in @null_values) and (LBTESTCD not in @null_values) and (LBORRES not in @null_values) and 
                            (LBSTNRC in @null_values))"
        }
