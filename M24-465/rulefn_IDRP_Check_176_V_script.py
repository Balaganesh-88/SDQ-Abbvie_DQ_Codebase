"""
Rule Name: rulefn_IDRP_Check_176_V
Domains: dv_sss_v_df
logic:If _R_SUBJYN_T_SSS = Y and DVDECOD = NULL , query should fire.
"""

def rulefn_IDRP_Check_176_V(query_text, dv_sss_v_df):
    payload_records = []
    prim_df = dv_sss_v_df
    if prim_df.shape[0] > 0:
        args_dict = {
            'p_form_ls': ['DV_SSS_F'],
            'p_visit_ls': [],
            'logic_text': " (DVDECOD.isna() or DVDECOD in @null_values)",
        ...
        if len(prim_df)>0:
            p_flag, prim_df = udf_glbl_yes_no_check(prim_df,'_R_SUBJYN_T_SSS','yes','==')
        if len(prim_df) > 0:
