"""
Rule Name: rulefn_IDRP_Check_139
Domains: ho_f_df
LOGIC:HOOCCUR = Y and _V_VSORRES_T_WEIGHT = Null. Query should fire
"""

def rulefn_IDRP_Check_139(query_text, ho_f_df):
    payload_records = []
    prim_df = ho_f_df
    if prim_df.shape[0] > 0:
        args_dict = {
            'p_form_ls': ['SAE_SUP2_F'],
            'p_visit_ls': [],
            'logic_text': " (_V_VSORRES_T_WEIGHT.isna() or _V_VSORRES_T_WEIGHT in @null_values)",
        ...
        if len(prim_df)>0;
            p_flag, prim_df = udf_yes_no_check(prim_df,'HOOCCUR','y','==')
        if len(prim_df) > 0:
            if refid_filter:
